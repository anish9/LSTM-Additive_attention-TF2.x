...
parser.add_argument(
    "--input_id", type=str, required=True, help="input column name from CSV"
)
parser.add_argument(
    "--output_id", type=str, required=True, help="output column name from CSV"
)
parser.add_argument(
    "--data_count", type=int, required=True, help="total rows to be selected"
)
parser.add_argument(
    "--batchsize", type=int, default=128, required=False, help="batchsize for training"
)
parser.add_argument(
    "--epochs", type=int, default=60, required=False, help="epochs to train"
)
parser.add_argument(
    "--units",
    type=int,
    default=64,
    required=False,
    help="Recurrent number of cells ex:(lstm units)",
)
parser.add_argument(
    "--embeds", type=int, default=32, required=False, help="embedding vector length"
)
parser.add_argument(
    "--ckpt_dir", type=str, required=True, help="checkpoint storing directory"
)
parser.add_argument(
    "--save_frequency",
    type=int,
...
# saving a tokenizer
inp_tok, inputs = data_utils.tokenize_sentence(
    dataset[args["input_id"]],
    save=True,
    filename=args["input_id"] + ".json",
    num_words=16000,
)
inp_tok, inputs = data_utils.tokenize_sentence(
    dataset[args["output_id"]],
    save=True,
    filename=args["output_id"] + ".json",
    num_words=16000,
)
