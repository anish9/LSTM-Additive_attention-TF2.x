...
inp_tok, inputs = data_utils.tokenize_sentence(
    dataset[load_config["input_node"]], json_file=load_config["input_vectorizer"]
)
inputs, input_config = data_utils.seq_preprocess(inputs, "source")
out_tok, outputs = data_utils.tokenize_sentence(
    dataset[load_config["output_node"]], json_file=load_config["output_vectorizer"]
)
...
data = data.shuffle(100).batch(
    batch_size=load_config["batch_size"], drop_remainder=True
)
...
def run_trainer(
    ...
    epochs,
    ckpt_writer,
    mask_token=output_config["mask_token"],
):
        ...
        for i, (x, y) in enumerate(dataset):
            ...
            with tf.GradientTape() as t:
                ...
                for steps in range(y_f.shape[-1]):
                    ...
                    loss_value = model_utils.loss_function(
                        y_f[:, steps], dec_out, mask_val=mask_token
                    )
                    loss += loss_value
