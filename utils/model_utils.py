...
class Encoder(tf.keras.models.Model):
    def __init__(self,lstm_units,embed_dim,vocab,batch_size):
        ...
        self.cell1  = layers.LSTM(self.lstm_units,return_sequences=True,return_state=True)
        self.drop2  = layers.Dropout(0.05)
        self.cell2  = layers.LSTM(self.lstm_units,return_sequences=True,return_state=True)
...
def predict_samples(batch,ENCODER_MODEL,DECODER_MODEL,inp,tar,inp_tok,out_tok):
    ...
    for steps in range(30):
        dec_val_out,dec_val_state = DECODER_MODEL(dec_val_inp,enc_val_state,enc_val_out)
...
def inference(sent,infer_enc,infer_dec,input_config,output_config,json_file,out_tok,batch=1):
    #inference test time
    try:
        ...
        inputs_ = tf.keras.preprocessing.sequence.pad_sequences(inputs_,maxlen=input_config["length"],padding="post",value=input_config['mask_token'])
