...
class Data_block():
    ...
    def get_tokenizer(self,text,num_words,filename=None,save=False):
        tokenizer = tf.keras.preprocessing.text.Tokenizer(num_words=num_words,filters=self.filters,
                                                   lower=True, split=' ',oov_token=0)
    ...
    def restore_tokenizer(self,filename):
        tokenizer = tf.keras.preprocessing.text.tokenizer_from_json(json.load(open(filename,"r")))
        return tokenizer
...
def tokenize_sentence(inp_text,**kwargs):
    ...
    if "json_file" in files.keys():
        ...
        tokenizer_ = data_.get_tokenizer(text=inp_text,num_words=files["num_words"],save=files["save"],filename=files["filename"])
    else:
...
def seq_preprocess(seq,target_):
    ...
    if target_=="target":
         prep = [[pad_dict["<bos>"]]+seq[t]+[pad_dict["<eos>"]] for t in range(len(seq))]
    
    max_length = max([len(j) for j in prep])
    prep = pad_sequences(prep,maxlen=max_length,padding="post",value=max(sum(seq,[]))+3)
